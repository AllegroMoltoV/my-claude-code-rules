...
#
# 動作
#   decision=block を返し、記録を促す通知文を Claude へ届ける。
#
#   ただし、応答の終了を阻止できるかどうかはホスト側の実装に依存する。
#   Claude Code 2.1.224 で実測したところ、この block は suggestion として扱われ、
#   preventedContinuation は false だった。通知文はモデルに届くが、モデルは
#   そのまま応答を終えられる。「必ず記録させる」仕組みではなく、
#   「記録漏れに気づかせる」仕組みとして設計すること。
#
# 多重発火の抑止
#   ホストが応答の完了を止める実装であれば、条件が真のままだと無限ループになりうる。
#   止められない実装でも、毎ターン通知が出ると邪魔になる。
#   どちらの場合にも効くよう、次の 3 段で抑止している。
#
#   1. stop_hook_active : 既に一度発火した状態で再入した場合は即座に黙る。
#   2. レート制限       : 発火する直前に stamp を書き、COOLDOWN_SEC の間は必ず黙る。
#                         Claude が記録を怠っても、次の Stop は素通りする。
#                         したがって発火は COOLDOWN_SEC あたり最大 1 回に限られる。
...
#
# 誤検知の扱い
#   記録すべきことが無いターンでも発火しうる。その場合の損失は通知が 1 回出るだけで、
