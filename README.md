# my-claude-code-rules

Claude Code で使用する開発ルール集です。プロジェクト管理、テスト、ドキュメント作成における規約を定義しています。

## 背景

Claude Code を活用した開発では、コミットメッセージの書き方やドキュメントの文体など、プロセス全体に一貫性が求められます。本リポジトリは、こうした規約をルールファイルとして管理し、プロジェクト間で再利用できるようにしたものです。

Claude Code はコンテキスト圧縮 (compact) 時に `CLAUDE.md` と `.claude/rules/` の内容だけを再注入します。会話中に読み込んだファイルは圧縮後に抜け落ちるため、ルールは `.claude/rules/` に配置する必要があります。

## 内容物

### rules/

対象プロジェクトの `.claude/rules/` にコピーして使用するルールファイルです。

| ファイル | 概要 |
|---|---|
| `project.md` | Git の運用ルール、計画の承認フロー、Bash 実行時の注意点など、プロジェクト管理全般の規約 |
| `testing.md` | テスト失敗時の対処方針やカバレッジ目標など、テストに関する規約 |
| `documentation.md` | 日本語技術文書の文体、表記ルール、構成方針に関するスタイルガイド。`**/*.md` に適用 |
| `working-principles.md` | 事実の検証、調査の先行、報告文の書き方、外部操作や破壊的操作の扱いなど、作業の進め方に関する行動原則 |
| `beads.md` | Beads によるタスク進行管理の運用ルール。状態の正本、記録するタイミング、決定の残し方 |

### .prompts/

対象プロジェクトの `.prompts/` にコピーして使用するファイルです。

| ファイル | 概要 |
|---|---|
| `INIT.md` | 初回セットアップ用プロンプト。要件の記入欄、質問の洗い出し、作業ディレクトリの作成を指示する |

### skills/

Claude Code のスキルです。`~/.claude/skills/` に配置して使用します。

| ディレクトリ | 概要 |
|---|---|
| `project-bootstrap/` | 新しいプロジェクトの初期セットアップを一通り行うスキル。環境の整備、要件の転記、現状の調査、長期タスクの登録、不明点の洗い出しまで進める |

### scripts/

セットアップと検証のスクリプトです。POSIX シェルで実行します。

| ファイル | 概要 |
|---|---|
| `bootstrap.sh` | プロジェクトの初期セットアップ。ルールとスキルの配置、git の初期化、除外設定、作業ディレクトリの作成を行う。何度実行しても結果が変わらない |
| `lib/rules.sh` | ルールとスキルの配置を扱う共通処理。シンボリックリンクを作れるかを実測し、作れない環境ではコピーへ切り替える |
| `setup-beads.sh` | Beads をグローバルに導入する。`~/.claude/settings.json` にフックを追加する |
| `teardown-beads.sh` | Beads の導入を取り消す。`--restore` で `settings.json` をバックアップから復元する |
| `verify-beads.sh` | Beads の動作を検証する。使い捨てのリポジトリを作り、終了時に削除する |
| `beads-stop-nudge.sh` | 記録漏れを防ぐフック。作業中のまま長時間更新されていないタスクを検出し、記録を促す |

### その他

| ファイル | 概要 |
|---|---|
| `exclude` | `.git/info/exclude` の参照用コピー |
| `.gitattributes` | スクリプトと `exclude` の改行を LF に固定する設定。CRLF ではシェルがスクリプトを実行できない |

## 前提

`scripts/` 配下のスクリプトは POSIX シェルで動きます。シェルが使えない環境では、後述の手動配置を行ってください。

`setup-beads.sh` は `bd` と `jq` を必要とします。スクリプトはこれらを導入しません。未導入の場合は導入方法を Claude Code に尋ねてください。環境ごとに手段が異なるため、本リポジトリでは特定の方法を指定しません。

## 使い方

### 環境変数の設定

`MY_CLAUDE_RULES` に、本リポジトリを複製した位置を設定してください。スキルと `INIT.md` はこの変数からスクリプトの位置を解決します。

Claude Code のツール呼び出しは非対話シェルで動きます。そのため `~/.bashrc` のような対話シェル向けの設定ファイルに書いても、値が届かないことがあります。`~/.claude/settings.json` の `env` は Claude Code のセッションに対して環境変数を設定する仕組みで、シェルの初期化に依存しません。

```json
{
  "env": {
    "MY_CLAUDE_RULES": "<リポジトリのパス>"
  }
}
```

### セットアップスクリプトの実行

対象プロジェクトのディレクトリで実行してください。同じ状態に対して何度実行しても、配置内容は変わりません。実行ごとに時刻付きのログが `.logs/` に増えます。

```bash
bash "${MY_CLAUDE_RULES}/scripts/bootstrap.sh"
```

スクリプトはルールとスキルを `~/.claude/` へ配置します。対象プロジェクトには、git の初期化、除外設定の追記、作業ディレクトリの作成、`.prompts/INIT.md` の配置を行います。リポジトリ自身には書き込みません。

Beads を使わないプロジェクトでは、先に `touch .beads-optout` を実行してください。

配置は既定でシンボリックリンクです。原本を編集すれば即座に反映されます。既にリンクがあり向き先が違う場合は、黙って現在のリポジトリへ付け替えます。リポジトリを移した直後は必ずこの経路を通ります。

**シンボリックリンクを作れない環境では、自動的にコピーへ切り替わります。** その場合は `(この環境は symlink 非対応)` と表示されます。

コピーではリポジトリを更新しても `~/.claude/` へ自動では反映されません。`git pull` のあとに `bootstrap.sh` を再実行してください。内容が異なるファイルは原本の内容で上書きされ、`[run ] 原本の内容で更新した` と表示されます。

このため `~/.claude/rules/` のファイルを直接編集しても、次回の実行で失われます。環境ごとに内容を足す場合は、原本側で管理してください。

### プロジェクトへのコピー

`bootstrap.sh` に `--with-project-rules` を付けると、対象プロジェクトの `.claude/rules/` へルールの実体をコピーします。他の機械へ持ち出す用途のための機能です。

**コピーは上流の更新に追随しません。** 一度置いたあとにリポジトリを更新しても、そのプロジェクトのコピーは古いままです。実測では、2026 年 4 月から 5 月に配置された 21 プロジェクト分のコピーが、当時のコミットのまま残っていました。

またプロジェクトの `.claude/rules/` は、`~/.claude/rules/` と同時に読み込まれます。セッションの記録で、同じルールが `Project` と `User` の 2 系統で注入されることを確認しています。`bootstrap.sh` は `.claude/rules/` を見つけると警告を出します。

持ち出しの用途が無ければ、このオプションは使わないでください。

### 手動での配置

シェルが使えない環境では、`rules/` 配下のファイルを対象プロジェクトの `.claude/rules/` にコピーしてください。`.claude/rules/` は Claude Code がセッション開始時に自動で読み込み、コンテキスト圧縮 (compact) 後もルールが維持されます。

あわせて `.git/info/exclude` に本リポジトリの `exclude` の各行を追記し、`.prompts/INIT.md` をコピーしてください。追記であって置き換えではありません。`.git/info/exclude` には Git が生成したコメントや、そのリポジトリ固有の除外設定が既にある場合があります。`bootstrap.sh` も不足している行だけを追記します。

### 初回セットアップ

`.prompts/INIT.md` の要件、前提と制約、完了の定義を記入したうえで、Claude Code にこのファイルの実行を依頼してください。Claude Code が現状を調査し、不明点を `.prompts/QUESTIONS.md` に洗い出します。回答を待ってから次の作業に進みます。

`project-bootstrap` スキルを新規登録した直後は、Claude Code を再起動するまでスキルが認識されません。

### Beads の導入

タスク進行管理を使う場合のみ実行します。

```bash
bash "${MY_CLAUDE_RULES}/scripts/setup-beads.sh"
```

スクリプトは `~/.claude/settings.json` にフックを 3 件追加します。追加前のバックアップを `settings.json.pre-beads.bak` として作成します。

このうち 1 件は Stop フックです。作業中のまま 15 分以上更新されていないタスクがあると、応答を終える直前に記録を促す通知を出します。発火は 15 分あたり最大 1 回に制限されており、間隔は環境変数 `BD_STOP_STALE_SEC` と `BD_STOP_COOLDOWN_SEC` で変えられます。

このフックは `decision: block` を返しますが、応答の終了を止められるかはホスト側の実装によります。Claude Code 2.1.224 で実測した範囲では、これは提案として扱われ、通知文はモデルに届くものの応答はそのまま終わりました。記録を強制する仕組みではなく、記録漏れに気づかせる仕組みとして扱ってください。

取り消しは次のコマンドで行います。

```bash
bash "${MY_CLAUDE_RULES}/scripts/teardown-beads.sh" --restore
```

`--restore` は `settings.json` をバックアップからバイト単位で復元し、`bd` が `CLAUDE.md` に追記した管理ブロックも取り除きます。引数なしで実行した場合は、追加したフックだけを削除します。

次のものは残るため、必要なら手作業で取り除いてください。

- `bd metrics off` で停止した匿名利用統計の設定。無断で送信を再開させないため、自動では戻しません。再開するには `bd metrics on` を実行してください
- `settings.json.pre-beads.bak` と `.logs/` 配下のログ
- `bd` コマンド本体と各リポジトリの `.beads/`。タスクの状態が失われるためです

`~/.claude/rules/beads.md` は削除しますが、内容が原本と異なる場合は警告だけ出して残します。独自に加えた変更を黙って失わせないためです。

取り消しにも `bd` と `jq` が必要です。不足している場合は、設定を変更せずに中止します。

## ライセンス

MIT License。詳細は [LICENSE](LICENSE) を参照してください。

## 更新履歴

| バージョン | 日付 | 内容 |
|---|---|---|
| v2.6.0 | 2026-08-08 | macOS 側での実施結果を反映。`setup-beads.sh` と `teardown-beads.sh` の Stop フックの判定を、コマンド文字列の完全一致からスクリプト名へ変更。リポジトリを移設すると古いパスのエントリが残り、同じ通知が 2 回発火する問題を解消。`working-principles.md` に「エラーと証拠なしを同じ 0 件にしない」を追加。README に、リンクの向き先が違う場合の動作と、`--with-project-rules` で置いたコピーが上流に追随しない点を追記。プロジェクトの `.claude/rules/` がグローバルと同時に読み込まれることを実測にもとづき明記。`.beads-optout` を追加し、本リポジトリで Beads を使わない意図を明示 |
| v2.5.1 | 2026-08-08 | `.gitattributes` に `*.md` を追加し、Markdown の改行も LF に固定。チェックアウトのたびに `bootstrap.sh` が誤った更新報告を出す問題を解消。Stop フックの説明を実測結果に合わせて修正。スクリプトは `decision: block` を返すが、Claude Code 2.1.224 ではこれが提案として扱われ、応答の終了は阻止されない。記録を強制する仕組みではなく、記録漏れに気づかせる仕組みであることを明記。挙動そのものは変更していない |
| v2.5.0 | 2026-08-08 | `working-principles.md` に 6 節を追加。画像の取り扱い、単発の観測から一般化しない、経過観察を提案する前に既存データを調べ切る、対策は強度順に示す、誤りは冒頭で認定する、機構の検証を運用の検証と混同しない。`documentation.md` に 3 節を追加。一般語をカタカナで書く判定基準、見出しは文にしない、事実と推測を分けて書く。追加した判定基準にしたがい、追跡対象の文書から一般技術語の英語表記をカタカナへ改めた。`teardown-beads.sh` を修正し、`--restore` でも `bd` の統合を削除するようにした。従来は `CLAUDE.md` の管理ブロックが残っていた。あわせて依存コマンドの事前確認、配置先が原本と異なる場合の保護、匿名利用統計を自動では戻さない方針を追加 |
| v2.4.0 | 2026-08-07 | Beads によるタスク進行管理と、プロジェクト初期セットアップの自動化を追加。`rules/beads.md`、`skills/project-bootstrap/`、`scripts/` のスクリプト 6 本を新規追加。`.prompts/INIT.md` を、要件と前提と完了の定義を記入する形へ置き換え、手順はスキルに委譲。スクリプトの位置を環境変数 `MY_CLAUDE_RULES` で解決する方式に変更。`.gitattributes` を追加し、シェルスクリプトと `exclude` の改行を LF に固定。CRLF が原因で除外設定の追記が冪等でなくなる問題を解消。`setup-beads.sh` から特定のパッケージ管理への依存を除去し、依存コマンドの確認のみを行う形に変更 |
| v2.3.0 | 2026-07-18 | 過去セッションのメモリから汎用的な行動原則を抽出し `working-principles.md` を追加。日本語出力に不自然な半角スペースを入れない原則を、ターミナル応答も含めた全出力に効くグローバル原則として `working-principles.md` に明記 (`documentation.md` は `**/*.md` にしか効かないため)。`documentation.md` 側にも半角スペースの適用範囲 (docs とターミナル / UI / 投稿文案の切り分け) を追記。曖昧で広い意味を持つ言葉や、英語から直訳したような表現を避ける規約を明確化。`testing.md` にモックの限界に関する注意を追加。`project.md` に多段階プランの初手集約とサブエージェント委譲の方針を追加。ツール呼び出しの XML 露出 (`court`・`course`・`count` 表示) によるセッション健全性の低下と、その予防・対処を `working-principles.md` に記載 |
| v2.2.0 | 2026-05-21 | プラン作成後に懸念点を `.prompts/DISCUSSIONS/` に洗い出し、推奨案を含む複数案を提示するフローを追加 |
| v2.1.0 | 2026-05-20 | INIT.md に調査先行ステップを追加。プラン品質基準・自律的完結方針・docs 蓄積ルール・TDD 原則を追加 |
| v2.0.1 | 2026-03-03 | `.prompts/INIT.md` を追加。`.prompts/` を Git 追跡対象に変更 |
| v2.0.0 | 2026-03-03 | `.claude/rules/` 方式に移行。`SKILLS/` と `INIT.md` を廃止し `rules/` に統合。リポジトリ名を `my-claude-code-rules` に変更 |
| v1.0.0 | 2026-02-25 | 初の安定版。`git -C` コマンドの禁止ルールを追加 |
| v0.1.1 | 2026-02-23 | 使い方の手順を具体化 |
| v0.1.0 | 2026-02-23 | 初版。PROJECT-SKILL、TEST-SKILL、DOC-SKILL を作成 |
